...
This module trains and evaluates Variational Autoencoders (VAEs) for multiple data perturbation methods,
corruption levels, and training modes. It trains across  across
combinations of sample sizes, corruption probabilities, perturbation methods, and VAE modes.
SWEEP_VAE does the same thing but as a grid over hyper parameters as well.

Modes supported:
- "normal": standard VAE loss
- "masked": loss computed only on uncorrupted features
- "mask_aware": uses the corruption mask as part of the input

Perturbation methods supposed:
1. Gaussian noise
2. Dropout with false positives
3. Log-normal noise

For each configuration, the script:
- Loads corrupted and clean data
- Trains a VAE with early stopping
- Saves the best model and its reconstructed output
- Plots and saves training loss curves
- Logs the results in a summary CSV

Relevant paths for models, data, and plots are fixed for my system

Note to self:
- `VAE.py` handles only standard (normal) VAE training.
- `U2_VAE.py` supports multiple modes but assumes a single perturbation method.
- `xVAE.py` supports multiple modes and multiple perturbation methods.
- `SWEEP_VAE.py` adds full grid search capabilities for evaluating and comparing configurations.
"""
...
corruption_probs = [0.05, 0.10, 0.30, 0.50]
    ...
    for method in perturbation_methods:
        for N in sample_sizes:
            ...
            for P in corruption_probs:
                ...
                torch.save(best_model_state, os.path.join(model_path, model_name))

                # Save reconstructed X
                with torch.no_grad():
                    XY = XY.to(device)
                    recon_X, _, _ = vae(XY)
                torch.save(recon_X.cpu(), os.path.join(data_path, f"X_{mode_short}_{method}_{N}_{P_int}.pt"))
                ...
                plt.savefig(os.path.join(plot_path, f"loss_{mode_short}_{method}_{N}_{P_int}.png"))
                plt.close()
                ...
                print(f"Trained VAE ({mode}, method {method}) for N={N}, P={P_int}% | Best loss: {best_loss:.4f} | Epoch {best_epoch+1}")
